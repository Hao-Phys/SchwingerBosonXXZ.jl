function link_phase(link::Int, q_reshaped::Vec3)
    if link == 1
        return 1 + exp(-2π*im * q_reshaped[2]) + exp(-2π*im * (q_reshaped[1]+q_reshaped[2]))
    elseif link == 2
        return 1 + exp( 2π*im * q_reshaped[2]) + exp(-2π*im * q_reshaped[1])
    elseif link == 3
        return 1 + exp( 2π*im * q_reshaped[1]) + exp( 2π*im * (q_reshaped[1]+q_reshaped[2]))
    else
        error("Invalid link index: $link")
    end
end

Q_link(link::Int, σ, J₊, J₋, Bs, Cs, α_dcoups) =  0.5 * (J₊*(1-α_dcoups[1])*Bs[link] + σ*J₋*(1-α_dcoups[2])*Cs[link])
P_link(link::Int, σ, J₊, J₋, As, Ds, α_dcoups) = -0.5 * (J₊*(1+α_dcoups[1])*σ*As[link] + J₋*(1+α_dcoups[2])*Ds[link])

function dynamical_matrix!(D::Matrix{ComplexF64}, sbs::SchwingerBosonSystem, q_reshaped::Vec3)
    D .= 0.0
    (; J, Δ, mean_fields, h_SB, θs, S, α_dcoups) = sbs
    J₊ = J * (Δ + 1) / 2
    J₋ = J * (Δ - 1) / 2

    # Extracting mean fields
    As = mean_fields[1:3]
    Bs = mean_fields[4:6]
    Cs = mean_fields[7:9]
    Ds = mean_fields[10:12]
    μ0s = mean_fields[13:15]

    # Block structure of the dynamical matrix
    D11 = view(D, 1:6, 1:6)
    D22 = view(D, 7:12, 7:12)
    D12 = view(D, 1:6, 7:12)
    D21 = view(D, 7:12, 1:6)

    for α in 1:3
        @. D11[2α-1:2α, 2α-1:2α] += -S*h_SB*(sin(θs[α])*σs[1] + cos(θs[α]) * σs[3])
        @. D22[2α-1:2α, 2α-1:2α] += -S*h_SB*(sin(θs[α])*σs[1] + cos(θs[α]) * σs[3])

        phase = link_phase(α, q_reshaped)
        for σ in 1:2
            sign = σ == 1 ? 1 : -1
            i = (α-1) * 2 + σ
            j = mod1(i+2, 6)

            # Below we follow the convention in Sunny to define the dynamical matrix
            # D11 and D22
            D11[i, j] += Q_link(α, sign, J₊, J₋, Bs, Cs, α_dcoups) * phase
            D11[j, i] += conj(Q_link(α, sign, J₊, J₋, Bs, Cs, α_dcoups)) * conj(phase)
            D22[i, j] += conj(Q_link(α, sign, J₊, J₋, Bs, Cs, α_dcoups)) * phase
            D22[j, i] += Q_link(α, sign, J₊, J₋, Bs, Cs, α_dcoups) * conj(phase)

            # D21 and D12
            increment = σ == 1 ? 3 : 1
            j = mod1(i+increment, 6)
            D12[i, j] += P_link(α, sign, J₊, J₋, As, Ds, α_dcoups) * phase
            D12[j, i] += P_link(α, sign, J₊, J₋, As, Ds, α_dcoups) * conj(phase)
            D21[i, j] += conj(P_link(α, sign, J₊, J₋, As, Ds, α_dcoups)) * phase
            D21[j, i] += conj(P_link(α, sign, J₊, J₋, As, Ds, α_dcoups)) * conj(phase)

            # Diagonal terms
            D11[i, i] += -real(μ0s[α])
            D22[i, i] += -real(μ0s[α])
        end
    end
end